...
def getTestResumeVariableT0(tests,barrierN=0,barrierP=0) :
    """
    * Input : 
        - tests : library of libraries; each internal library contains the results for a test with the variable
                  parameter (let's say t0).
                  The internal libraries are indexed by the pair of coefficients (cL,cR), and have the results
                  for the test made on ONE time step
                  For example, the test with the parameter t0, for the coefficients (cL,cR)has the format
                  tests[str(t0)][str((cL,cR))] = (number of iterations,error in CV,
                                                  error in function of iteration (array),
                                                  error in CV in Omega1,
                                                  error in function of iteration in Omega1(array),
                                                  error in CV in Omega2,
                                                  error in function of iteration in Omega2(array))
        - barrierN : minimum value for positive coefficients to produce the resume.
                    e.g. barrierN = 0.2 if we want to study only the positive coefficients >= 0.2 
        - barrierP : maximum value for negative coefficients to produce the resume       
        
    * Output :
        - resumeT0 : a lighter version of the input library. It is a simple library indexed by the variable
                     parameter (let's say t0).
