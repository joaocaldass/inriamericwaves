...
def EFDSolver(h,u,dx,dt,t,order,BCfunction,BCparam=None,periodic=False,ng=2):
    ...
    return u2
def EFDSolverFM4(h,u,dx,dt,t,order,BCfunction,BCparam=None,periodic=False,ng=2):
...
def EFDSolverO2hu(h,u,dx,dt,t,BCfunction,order,BCparam=None,periodic=False):
    ...
    M[-2,-2] = 1. + 3./4.*h2x[-2]/dx - 2./3.*h[-2]*h[-2]/(dx*dx)
    M[-2,-3] = -h2x[-2]/dx + 5./3.*h[-2]*h[-2]/(dx*dx)
    M[-2,-4] = 1./4.*h2x[-2]/dx - 4./3.*h[-2]*h[-2]/(dx*dx)
    M[-2,-5] = 1./3.*h[-2]*h[-2]/(dx*dx)
