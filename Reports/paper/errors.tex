...
%\indent The following scheme briefly illustrates these remarks :

\tikzstyle{node}=[minimum width=0cm, minimum height=0cm, rectangle,
			draw]
			
%\begin{tikzpicture}
%	\node[node] (A) at (0,4) {$u$  :   Problem solved in the infinite domain};
%	\node[node] (B) at (0,2) {$\overline{u}$ :  Problem solved in the finite computational domain};
%	\node[node] (C) at (0,0) {$\tilde{u}$ :  Problem solved in the finite computational domain divided in subdomains};
%	
%	\draw[->] (A) -- (B) node[midway,right]  {Work of \cite{zheng2008} and \cite{besse2015}};
%	\draw[->] (B) -- (C) node[midway,right]  {Our work};
%\end{tikzpicture}
