...
\indent In \ref{eq:genericIBC}, $\mathcal{B}_i$ denotes the operator of the IBC. This operator allows the construction of of more general Schwarz methods : in the original one, the IBC's are Dirichlet conditions (i.e. ,$\mathcal{B}_i(u) = u$  ) \cite{Japhet2003,LionsI990}
...
\begin{minipage}{.5\linewidth}
\begin{center}
	\includegraphics[scale=.5]{{figures/FinalFigures/OptimalCoefVarDxDtFinalVersionMarshal2em2OnlyDt.png}}
	\captionof{subfigure}{Fixed $\Delta x = \frac{12}{250}$ }
\end{center}
\end{minipage}
\begin{minipage}{.5\linewidth}
\begin{center}
	\includegraphics[scale=.5]{{figures/FinalFigures/OptimalCoefVarDxDtFinalVersionMarshal2em2OnlyDx.png}}
	\captionof{subfigure}{Fixed $\Delta t = 0.02$ }
\end{center}
\end{minipage}
...
\indent The figure \ref{fig:optimalCoefVarDxDtCorrectN} suggests a dependence of the optimal coefficient on $(\Delta t)^\nu$ and $(\Delta x)^\eta$, with $0 \leq \nu \leq 1$ and $\eta < 1$. In fact, performing some regressions with $\Delta t $ or $\Delta x$ fixed, we could conclude that $\nu = \frac{2}{3}$ and $\eta = 1$ provide really well-fitted regression curves (with the coefficients of determination $R^2$ bigger than 0.99), both for the negative and the positive coefficients (although each one of these cases correspond to different pairs $(\nu,\eta)$). Therefore, we will seek to model a function
